#!/usr/bin/env python3
"""
Seed events untuk testing event sourcing (FIXED)
...
# Valid enum values untuk institution type (berdasarkan database)
VALID_INSTITUTION_TYPES = ["GOVERNMENT", "PRIVATE", "INTERNATIONAL", "NGO"]
...
async def create_test_case(conn, institution_id: str) -> str:
    ...
    # Check if cases table has priority column
    try:
        await conn.execute("""
        ...
        # Fallback tanpa priority column
        await conn.execute("""
            INSERT INTO cases (id, title, description, status, category, institution_id, created_at, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
        """, case_id, case_data["title"], case_data["description"], "ACTIVE", 
            case_data["category"], institution_id)
...
async def main():
    print("=" * 70)
    print("NEMESIS MADINA - SEED EVENTS SCRIPT (FIXED)")
    print("=" * 70)
    
    conn = await asyncpg.connect(DATABASE_URL)
    
    # Check existing institution or create new one
    institution_id = None
    result = await conn.fetchrow("SELECT id FROM institutions WHERE code = 'AUDIT001'")
    
    if result:
        institution_id = result["id"]
        print(f"\n📋 Using existing Institution ID: {institution_id}")
    else:
        # Create new institution with valid type
        institution_id = str(uuid.uuid4())
        valid_type = random.choice(VALID_INSTITUTION_TYPES)
        
        try:
            await conn.execute("""
                INSERT INTO institutions (id, name, code, type, created_at)
                VALUES ($1, $2, $3, $4, NOW())
            """, institution_id, "Audit Test Institution", "AUDIT001", valid_type)
            print(f"\n📋 Created Institution ID: {institution_id} (type: {valid_type})")
        except Exception as e:
            # Try without type if column doesn't exist
            await conn.execute("""
                INSERT INTO institutions (id, name, code, created_at)
                VALUES ($1, $2, $3, NOW())
            """, institution_id, "Audit Test Institution", "AUDIT001")
            print(f"\n📋 Created Institution ID: {institution_id} (no type)")
    ...
    for case_num in range(1, 6):
        ...
        # Create snapshots at every 10 events (if snapshots table exists)
        try:
            ...
            print(f"   ⚠️ Snapshots not created: {e}")
    ...
    try:
        ...
        print(f"📸 Snapshots table not available")
    ...
    print("\n📝 Test case IDs saved to: test_cases.txt")
