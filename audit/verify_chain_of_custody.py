...
async def audit_chain_of_custody():
    ...
    async for db in get_db():
        ...
        print(f"   Total custody transfers: {row[0] if row else 0}")
        print(f"   Evidence with custody records: {row[1] if row else 0}")
        
        results["passed"].append(f"Custody chain has {row[0] if row else 0} transfer records")
