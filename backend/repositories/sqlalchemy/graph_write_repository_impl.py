# backend/repositories/sqlalchemy/graph_write_repository_impl.py
...
R16.3.5: Uses dedicated business_key column, NOT extra_data workaround.
"""
...
class GraphWriteRepositoryImpl(GraphWriteRepository):
    ...
    R16.3.5: Uses dedicated business_key column.
    """
    ...
    async def save_aggregate(
        ...
        try:
            ...
            # R16.3.5: Use dedicated business_key column
    ...
    def _map_nodes_to_entities(self, aggregate: GraphAggregate) -> List[GraphEntity]:
        ...
        for node in aggregate.nodes:
            entity = GraphEntity(
                ...
                # R16.3.5: Dedicated column - NOT extra_data
                business_key=node.business_key,
                # R16.3.5: source_id tetap NULL sampai provenance audit
                source_id=node.source_id,
                # R16.3.5: extra_data PURE metadata - NO business_key
    ...
    async def _save_entities(
        ...
        for entity in entities:
            ...
            # R16.3.5: business_key is required
            if not entity.business_key:
                raise ValueError("GraphEntity MUST have business_key")
    ...
    def _build_entity_map(self, entities: List[GraphEntity]) -> Dict[str, str]:
        ...
        for entity in entities:
            ...
            if entity.business_key:
                entity_map[entity.business_key] = entity.id
        return entity_map
